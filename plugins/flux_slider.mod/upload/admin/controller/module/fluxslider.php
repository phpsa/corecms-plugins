...
class ControllerModuleFluxslider extends \Core\Controller {
    ...
    public function index() {
        ...
        $data['entry_crop'] = $this->language->get('entry_crop');
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('module/fluxslider', 'token=' . $this->session->data['token'], 'SSL')
            );
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('module/fluxslider', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
            );
        }

        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('module/fluxslider', 'token=' . $this->session->data['token'], 'SSL');
        } else {
            $data['action'] = $this->url->link('module/fluxslider', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
        }
        ...
        if (isset($this->request->post['crop'])) {
            $data['crop'] = $this->request->post['crop'];
        } elseif (!empty($module_info)) {
            $data['crop'] = $module_info['crop'];
        } else {
            $data['crop'] = '0';
        }
        ...
        $this->response->setOutput($this->render('module/fluxslider.phtml', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'module/fluxslider')) {
